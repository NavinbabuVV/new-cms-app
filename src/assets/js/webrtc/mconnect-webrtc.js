...
Janus.pauseAllTracks = function(stream) {
	try {
		// Try a MediaStreamTrack.stop() for each track
		var tracks = stream.getTracks();
		for(var mst of tracks) {
...
function Janus(gatewayCallbacks) {
				...
				var pluginHandle =
					{
						...
						getPlugin : function() {  console.log(plugin);  return plugin; },
						...
						data : function(callbacks) {  console.log(callbacks);  sendData(handleId, callbacks); },
	...
	function isVideoSendEnabled(media) {
		...
		console.log("isVideoSendEnabled:", media);
		if(!media)
			return true;	// Default
	...
	function isVideoSendRequired(media) {
		...
		console.log("isVideoSendRequired:", media);
		if(!media)
			return false;	// Default
	...
	function isVideoRecvEnabled(media) {
		...
		console.log("isVideoRecvEnabled:", media);
		if(!media)
			return true;	// Default
		if(media.video === false)
			return false;	// Generic video has precedence
...
function custommuteCall(){
	if(($('#mute_btn').length) > 0){
		html5vid.enabled = false;
		console.log(html5vid);
	$('.mute-btn').attr('id','muted_btn');
}
	else if(($('#muted_btn').length) > 0){
		html5vid.enabled = true;
		console.log(html5vid);
